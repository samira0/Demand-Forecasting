...
print(f"Лучшие параметры: {grid_search.best_params_}\nЗаняло: {(time.time() - start_time)/60:.2f} минут")
...
gradient_boosting_params = grid_search.best_params_
...
lgdm_params = grid_search.best_params_
...
cat_boost_params = grid_search.best_params_
...
estimators = [
    ...
    ('lgdm', LGBMRegressor(**lgdm_params)),
    ('xgb', XGBRegressor(**xgb_params))
]

stacking_kwargs = {
    'estimators': estimators,
    'final_estimator': GradientBoostingRegressor(**gradient_boosting_params)
}
...
y_pred = create_pred_list(model_class, X_train, y_train, X_test, y_test, **stacking_kwargs)
show_metrics(y_test, y_pred)
...
model = model_class(**stacking_kwargs)
model.fit(X_train, y_train)
y_pred = model.predict(X_test)
y_pred = pd.Series(y_pred, index=y_test.index)
show_metrics(y_test, y_pred)
...
# Лучшие значения метрик наблюдаются у GradientBoostingRegressor. У всех остальных рассмотренных моделей значиения метрик лучше, чем у наивных моделей и статистических. Использование стэкинга не улучшило качество предсказаний
