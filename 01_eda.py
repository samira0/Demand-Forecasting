...
# ## 1.3 Определение целевой переменной и типов признаков
...
# ## 1.4 Преобразование типов данных
...
# ## 1.5 Анализ и обработка пропущенных значений
...
# **Вывод:** Анализ показал, что пропущенные значения в датасете отсутствуют, что исключает необходимость применения методов восстановления данных.

# ## 1.6 Анализ дубликатов
...
# ## 1.8 Проверка логической согласованности данных
...
# ## 1.9 Детальный анализ проблемы с превышением продаж на 51.4%
...
# ### 1.9.2 Методологическое решение проблемы
...
# ### 1.9.3 Создание двух версий данных
...
# 
# В дальнейшем для моделирования используется только Версия B.

# ### 1.9.4 Создание копий данных для двух подходов

# In[13]:


df_version_a = df.copy()  # исследовательская версия (не используется в моделях)
df_version_b = df.copy()  # основная версия для дальнейшего анализа
...
# ### 1.9.5 Исключение признаков с утечкой информации
